...
source_url("https://gist.github.com/JohnsonHsieh/8389618/raw/qurey.R") # 戰績網查詢
id1 <- clean_lol("Toyz"); id2 <- clean_lol("AZB_TPA_Morning")
out <- list("Toyz"=id1$win); names(out[[1]]) <- id1$name
lapply(out, function(x) x[x>0])
```
...
lines(out2, col=2)
lines(out3, col=3)
legend("topleft", c("Toyz", "AZB_TPA_Morning", "TW.R_Wush"), 
...
---
## 媒體報導關聯性
```{r cache=TRUE, echo=FALSE, , fig.height=6.5, fig.width=6.5, fig.cap="news-association"}
library(igraph)
...
V(g)$label <- V(g)$name
plot.igraph(g, layout=layout1, vertex.label.family="STHeiti", vertex.label.cex=1.4, xlim=c(-0.8,1), ylim=c(-1,0.8))
```
